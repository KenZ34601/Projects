...
json_file = 'RFMLS_random_train375_test150.json'
#json_file = 'RFMLS_class12train16_test8.json' 
with open(json_file) as conf_json:
    config = json.load(conf_json, encoding = 'utf-8')
# the json file
#if len(sys.argv) < 2:
...
if config['tasks']['preprocess_trainset'] == 1:
    ...
    conf = {'data_dir':data_dir, 'result_dir':preprocess_dir, 'result_name': preprocess_fname, 'parameters': {'num_trace': config['preprocess']['num_trace_train'],'windowsize': config['preprocess']['windowsize'], 'stride': config['preprocess']['stride'], 'num_fft_windows': config['preprocess']['num_fft_windows']}}
    #print(conf)
    write_to_hdf5(conf)
if config['tasks']['train_anomaly_module'] == 1:
    preprocess_fname = data_name + '_' + num_trace_train
    traindata = preprocess_dir + preprocess_fname + '.hdf5'
    anomaly = str(config['nn_var']['anomaly_labels'])
    plot_dir = str(config['paths']['plot_dir'])
    call(["python2", "pfp.py", "--hidden_size", hidden_size, hidden_size, "--fc_size", fc_size, "--epoch", epochs, "--learning_rate", learning_rate, "--batch_size", batch_size, "--file", traindata, "--anomaly_labels", anomaly, "--dont_resume", "--checkpoint_dir", out_dir, "--gpu_id", gpu])
...
if config['tasks']['preprocess_testset'] == 1:
    ...
    conf = {'data_dir':data_dir, 'result_dir':preprocess_dir, 'result_name': preprocess_fname, 'parameters': {'num_trace': config['preprocess']['num_trace_test'],'windowsize': config['preprocess']['windowsize'], 'stride': config['preprocess']['stride'], 'num_fft_windows': config['preprocess']['num_fft_windows']}}
    write_to_hdf5(conf)

### nned to fix
if config['tasks']['test_anomaly_module'] == 1:
    preprocess_fname = data_name + '_' + num_trace_test 
    ...
    anomaly = str(config['nn_var']['anomaly_labels'])
    plot_dir = str(config['paths']['plot_dir'])
    call(["python2", "pfp.py", "--test", "--hidden_size", hidden_size, hidden_size, "--fc_size", fc_size, "--learning_rate", learning_rate, "--init_from", model, "--batch_size", batch_size, "--file", testdata, "--checkpoint_dir", out_dir, "--anomaly_labels", anomaly, "--gpu_id", gpu])
    config['output'].update({'test_result_dir':config['paths']['result_dir']+preprocess_fname+'/', 'log_test': 'output_test.log', 'roc_plts': plot_dir + '/roc.png'})
    with open(json_file,'w') as conf_json:
        json.dump(config, conf_json)
